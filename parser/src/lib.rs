...
pub fn parse(data: String) -> Result<Vec<Token>,SyntaxError> {
    ...
    loop{
        ...
        let push_token = match line.unwrap(){
            &"SET FREE"=>{iter.next();Some(Token::FreeWrite)},
            &"SET APPEND"=>{iter.next();Some(Token::AppendWrite)},
            &""=>{iter.next();None}
            _ => {iter.next();Some(Token::Error(SyntaxError::InvalidToken))}
        };
...
mod tests {
    ...
    fn line(){
        ...
        assert_eq!(p,Ok(vec![Token::DrawLine{
            x_start:10.0,
            y_start:10.0,
            x_end:20.0,
            y_end:20.0,
